...
echo -e "$OUTPUT" | mail -s "Уникальные адреса и их количество с $RUN_DATE по $CURRENT_DATE" alarin@narod.ru
# Топ запрошенных УРЛ и их количество
OUTPUT="$(awk '{if ($4 > "'$RUN_DATE'") print $11}' apache_log | sort -nr | uniq -c | sort -nr | head -n 30)"
echo -e "$OUTPUT" | mail -s "Топ запрошенных УРЛ и их количество с $RUN_DATE по $CURRENT_DATE" alarin@narod.ru
# Серверные ошибки
OUTPUT="$(grep "error"  ./error_log | awk '{if ($1 > "'$RUN_DATE'") print}')"
echo -e "$OUTPUT" | mail -s "Серверные ошибки с $RUN_DATE по $CURRENT_DATE" alarin@narod.ru
# Количество и коды ответов HTTP 
OUTPUT="$(awk '{if ($4 > "'$RUN_DATE'") print $9}' apache_log | sort -nr | uniq -c | sort -nr)"
echo -e "$OUTPUT" | mail -s "Количество и коды ответов HTTP с $RUN_DATE по $CURRENT_DATE" alarin@narod.ru
